...
# Lhy
from train import ClipCaptionModel_embedded_with_noise
...
def make_preds(data, model: ClipCaptionModel, out_path, tokenizer, dataset_mode, args=None):
    ...
    if dataset_mode == 0 or dataset_mode == 7 or dataset_mode == 8:
        ...
        images_root = './data/coco/val/val2014'
    elif dataset_mode == 1:
    ...
    for ii, d in enumerate(data):
        ...
        new_data.append({"image_id": d["image_id"],"caption": generated_text_prefix.lower()})
...
def load_data(dataset_mode):
    if dataset_mode == 0:
        with open(
                ...
                f'post_processed_karpthy_coco/val.json',
                'r') as f:
            data = json.load(f)
    elif dataset_mode == 1:
        with open(
...
def main():
    ...
    parser.add_argument('--trainable_noise', dest='trainable_noise', action='store_true', default=False)
    args = parser.parse_args()
    if ".json" in args.checkpoint:
        print("json file")
        return
    ...
    if args.choose:
        f = 0
        for index in ["000","001","005","009"]:
            if index in name:
                f = 1
                break
        if not f:
            print("index not choose")
            return
        else:
            print("index choose")
    ls = os.listdir(checkpoint_dir)

    print(args.dont_normalize_prefix)
    if not args.dont_normalize_prefix:
        out_path = f"{checkpoint_dir}/{checkpoint_dir.replace('/', '_')}-{name}.json" if (args.out == '') else args.out
        if f"{checkpoint_dir}-{name}.json" in ls:
            print("already exist")
            return
    else:
        out_path = f"{checkpoint_dir}/{checkpoint_dir.replace('/', '_')}-nonorm-{name}.json" if (args.out == '') else args.out
        if f"{checkpoint_dir}-nonorm-{name}.json" in ls:
            print("already exist")
            return
    ...
    # 改成这个
        model = ClipCaptionModel_embedded_with_noise(args.prefix_length, clip_length=args.prefix_length_clip, prefix_size=prefix_dim,
