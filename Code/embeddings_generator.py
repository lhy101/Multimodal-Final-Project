...
def main(clip_model_type, clip_model_name, out_path, annotations_path, images_path, fix_gender_imbalance, data_mode):
    ...
    for i in tqdm(range(len(data))):
        ...
        # Lhy
        # Also add image embeddings to the dataset
        if images_path != 'NoImgs':
            filename1 = f"./data/coco/train2014/COCO_train2014_{int(img_id):012d}.jpg"
            filename2 = f"./data/coco/val/val2014/COCO_val2014_{int(img_id):012d}.jpg"
            if os.path.isfile(filename1):
                ...
                not_found += 1
                print(f"Something wrong! Can't find {int(img_id):012d}.jpg")
                continue
...
def run_main():
    ...
    if args.dataset_mode == 0:
        out_path = f"./data/coco/COCO_train_set_not_norm.pkl"
        if not add_text_embedding:
            out_path = f"./data/coco/COCO_train_set_text_only_not_norm.pkl"
            print(f'Only text embeddings will be added to the dataset')
        annotations_path = f'post_processed_karpthy_coco/train.json'
