...
def noise_injection(x, is_image, variance=0.001, modality_offset=None, uniform_noise=False, dont_norm=False, not_text_only=False):
    if variance == 0.0:
        return x
    std = math.sqrt(variance)
    if not dont_norm:
...
class ClipCocoDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        is_image = (self.caption2embedding[item] % text_image_rate == 0)

        return tokens, mask, prefix, is_image

    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str = "gpt2",
                 normalize_prefix=False, use_image_embedding_as_clipcap=False, not_text_only=False):
...
class ClipCaptionModel_embedded_with_noise(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, prefix: torch.Tensor, mask: Optional[torch.Tensor] = None,
                labels: Optional[torch.Tensor] = None):
        prefix = torch.nn.functional.normalize(prefix, dim=1) + (torch.randn(prefix.shape, device=device) * 0.126911) + self.modality_offset
...
def train(dataset: ClipCocoDataset, model, args, warmup_steps: int = 5000, output_dir: str = ".", output_prefix: str = "", modality_offset_trainable: bool = False):
    ...
    loss_per_epoch_val = []
    modality_offset_per_epoch = []

    for epoch in range(args.begin_epoch, epochs):
        ...
        # Lhy
        for idx, (tokens, mask, prefix, is_image) in enumerate(train_dataloader):
            model.zero_grad()
            tokens, mask, prefix, is_image = tokens.to(device), mask.to(device), prefix.to(device, dtype=torch.float32), is_image.to(device, dtype=torch.bool)
            ...
            # Lhy
            if not modality_offset_trainable and not args.adv:
                prefix = noise_injection(prefix, is_image, args.noise_variance, modality_offset=modality_offset, 
                    uniform_noise=args.uniform_noise, dont_norm=args.dont_norm, not_text_only=args.not_text_only)
        ...
        if modality_offset_trainable:
            print("Dumping modality offset...")
            modality_offset_per_epoch.append(model.modality_offset.tolist())
            with open(os.path.join(output_dir, f"modality_offset.json"), 'w') as f:
                json.dump(modality_offset_per_epoch, f)
...
def main():
    ...
    if args.data == 'COCO':
        ...
        if args.use_image_embedding_as_clipcap:
            args.data = './data/coco/COCO_train_set_not_norm.pkl'
        if args.not_text_only:
            args.data = './data/coco/COCO_train_set_not_norm.pkl'
        else:
            args.data = './data/coco/COCO_train_set_text_only_not_norm.pkl'
        args.val_pt = ''
    ...
    # Lhy
    dataset = ClipCocoDataset(args.data, prefix_length, normalize_prefix=not args.dont_norm, 
        use_image_embedding_as_clipcap=args.use_image_embedding_as_clipcap,not_text_only=args.not_text_only)
    ...
    if args.only_prefix:
        ...
        if not args.modality_offset_trainable:
            model = ClipCaptionModel(prefix_length, clip_length=args.prefix_length_clip, prefix_size=prefix_dim,
                                  num_layers=args.num_layers, mapping_type=args.mapping_type)
            print("Train both prefix and GPT, using ClipCaptionModel")
        else:
            model = ClipCaptionModel_embedded_with_noise(prefix_length, clip_length=args.prefix_length_clip, prefix_size=prefix_dim,
                                  num_layers=args.num_layers, mapping_type=args.mapping_type)
            print("Train both prefix and GPT, using ClipCaptionModel_embedded_with_noise")
        sys.stdout.flush()
    ...
    # Lhy
    train(dataset, model, args, output_dir=args.out_dir, output_prefix=args.prefix, modality_offset_trainable=args.modality_offset_trainable)
